...
public class Usuario {
	...
	public Entrada proximaEntrada() {
	    return this.entradas.stream()
	            .filter(entrada -> entrada.getFecha().isAfter(LocalDate.now())) // Filtrar entradas futuras
	            .min((e1, e2) -> e1.getFecha().compareTo(e2.getFecha())) // Obtener la más cercana
	            .orElse(null); // Si no hay ninguna entrada futura, devuelve null
	}
