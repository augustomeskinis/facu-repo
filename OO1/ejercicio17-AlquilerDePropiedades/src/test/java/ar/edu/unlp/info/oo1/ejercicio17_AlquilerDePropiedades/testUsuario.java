...
public class testUsuario { // IGNORAR, FUE PARA PROBAR NOMAS NO VAS A ENTENDER NADA
	Propiedad propiedad;
	LocalDate from1;
	LocalDate to1;
	Reserva reserva1;
	...
	@BeforeEach
	public void setUp () {
		usuario = new Usuario ("augusto","4 e 3 y 5", 46345400);
	...
	public void testCrearReserva() {
		...
		assertEquals(false, usuario.reservar(propiedad, otroDateLapse));
		
		from3 = LocalDate.of(2025, 01, 11);
		to3 = LocalDate.of(2025, 01, 15);
		otroDateLapse = new DateLapse(from3,to3);
		
		assertEquals(true, usuario.reservar(propiedad, otroDateLapse));
